...
export default function OtpEntryScreen() {
  ...
  const handleSubmit = async () => {
    ...
    if (enteredOtp.length !== 6) {
      console.log("OTP length check failed:", enteredOtp.length);
      message.error("Please enter the complete 6-digit code.");
      return;
    }
    
    if (enteredOtp !== generatedOtp) {
      console.log("OTP mismatch - entered:", enteredOtp, "generated:", generatedOtp);
    ...
    try {
      ...
      if (result.success) {
        console.log("Signup successful, clearing localStorage and redirecting");
        // Clear localStorage
        ...
        // Don't redirect on signup failure, let user try again or go back
      }
    } catch (error) {
    ...
    try {
      const response = await fetch('https://textbelt.com/text', {
        ...
        body: JSON.stringify({
          ...
          message: `Your Upin verification code is ${newOtpCode}`,
